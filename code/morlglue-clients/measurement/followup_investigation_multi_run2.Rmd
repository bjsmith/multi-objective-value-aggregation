...
```{r message=FALSE}
source_path <- "../data/multirun_n100_start/"

file_list <- get_csv_file_list(source_path)

raw_activity <- get_raw_csv_activity(file_list,source_path)
...
Think we need to recode this stuff to use data.table instead of data.frame.
...
activity_long <- append_blackman_averaging(activity_long %>% group_by(Measure,EpisodeType,Agent,Environment,RunId))
...
We also need to calculate significance of the difference.

These will compare base case (TLO_A) with the other values. Let's keep it to just that. Perhaps we could do CIs after that.


```{r}

table_env[,.(EnvironmentClass,ObjectiveDimChange,EnvironmentValueChange,RunId,Agent,`R^*`)] %>% 
  
table_env[,.(EnvironmentClass,ObjectiveDimChange,EnvironmentValueChange,RunId,Agent,`R^*`)] %>% 
  .[,.N,.(EnvironmentClass,ObjectiveDimChange,EnvironmentValueChange,RunId,Agent)]
  

table_env %>% select(-`R^P`,-`R^A`)
table_env %>% dcast.data.table(
  EnvironmentClass+ObjectiveDimChange+EnvironmentValueChange+RunId~Agent
  ,value.var="R^*",fun.aggregate = mean) %>% 
  group_by(EnvironmentClass,ObjectiveDimChange,EnvironmentValueChange) %>%
  summarise(TLO_A_vs_SFLLA=t.test(TLO_A,SFLLA)$p.value)
...
table_env %>% dcast.data.table(
  ...
  ,value.var="R^*",fun.aggregate = mean) %>% 
  group_by(EnvironmentClass,ObjectiveDimChange,EnvironmentValueChange) %>%
  summarise(TLO_A_vs_SFLLA=sd(TLO_A,SFLLA))

t.test(c(1,2,3),c(11,6,88))
```
...
Let's add an SD to the graph below....
```{r fig.width=8}
