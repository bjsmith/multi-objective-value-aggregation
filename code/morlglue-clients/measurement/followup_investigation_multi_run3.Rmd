...
get_formatted_desc <- function(x,pval,control=NA){
  effect_direction_indicator<-""
  if (!is.na(control)){
    if(x>control){
        effect_direction_indicator <- "$\\uparrow$"
       }else{
        effect_direction_indicator <- "$\\downarrow$"
      }
  }
   
  paste0(formatC(x,digits=2,format="f"),ifelse(pval<0.05,paste0(effect_direction_indicator,"*"),""),ifelse(pval<0.01,"*",""),ifelse(pval<0.001,"*",""))
}
