...
table_env_long <- table_env %>% dcast.data.table(
  EnvironmentClass+EnvironmentDimChange+EnvironmentValueChange~Agent
...
setnames(table_env_long,"EnvironmentValueChange","Scale",skip_absent=TRUE)
setnames(table_env_long,"EnvironmentDimChange","Scale Change",skip_absent=TRUE)
#colnames(table_env_long) <- stringr::str_replace(colnames(table_env_long),"Envir","$TLO^A$")
table_env_long <- 
  table_env_long %>% mutate(Environment=str_replace(Environment,"BreakableBottles","Breakable\\nBottles"))
  
row_max <- apply(table_env_long[,4:ncol(table_env_long)],1,max)*0.9<table_env_long[,4:ncol(table_env_long)]
table_env_long %>%
  
  
  mutate(Scale=format(Scale,scientific = FALSE,drop0trailing = TRUE)) %>%
...
for (env_dim in c("pen","rew")){
  relevant_activity <- activity_means %>% dplyr::filter(EnvironmentDimChange %in% c("",env_dim))
    ...
    ggsave(filename=paste0(output_dir, "online performance_",env_dim,".pdf"),plot=envplot,device="pdf")
    ggsave(filename=paste0(output_dir, "online performance_",env_dim,".png"),plot=envplot,device="png")
