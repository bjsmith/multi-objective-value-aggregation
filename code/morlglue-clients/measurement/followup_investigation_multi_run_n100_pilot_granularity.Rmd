...
comparison_folder <- "multirun_n100_pilot_granularity_tuned_selected"
...
table_env$EnvironmentDimChange <- NULL
table_env[table_env$Environment=="Sokobanrew0.01","Environment"]<-"Sokoban"
```
...
table_env_control_run_means <- table_env_ctrl_agent[,
          ...
          ,by=.(Agent,Environment,RunId
                #,EnvironmentDimChange
                )]
...
significance_abridged <- significance_abridged  %>%
  mutate(Environment = as.factor(Environment)) %>% rename(
  LinearSum = LIN_SUM,
  `Primary Objective Granularity` = RewGranularity,
  `Alignment Objective Granularity` = PenGranularity,
  SFELLA = SFLLA1,
  EEBA = EEBA1,
  ) %>% use_display_names(abbreviate_envs = TRUE)
...
# activity_means_trimmed$AgentClass <- activity_means_trimmed$Agent
# activity_means_trimmed[activity_means_trimmed$Agent=="Sokobanrew0.01","AgentClass"]<-"Sokoban"

#now do 
for (granularity_dim in c("RewGranularity","PenGranularity")){
  if(granularity_dim=="RewGranularity"){
    constant_gran_dim = "PenGranularity"
    granularity_display_name <- "Primary"
  }else if (granularity_dim=="PenGranularity"){
    constant_gran_dim = "RewGranularity"
    granularity_display_name <- "Alignment"
  }
  ...
  relevant_activity <- activity_means_trimmed %>%
    dplyr::filter(
       (
        ...
        ((RewGranularity!=100 & PenGranularity!=100) | Agent==ctrl_agent)
    )%>% 
    use_display_names
  ...
  baseplot <- ggplot(relevant_activity,aes(x=GranularityLevel,y=`R^*`,fill=Agent,color=Agent,group=interaction(Agent,GranularityLevel)))+
    ...
    facet_wrap(~EnvironmentClass,scales = "free_y")+theme(axis.text.x = element_text(angle=30),legend.position = "bottom")+
      labs(
        x=paste0(granularity_display_name, " objective granularity magnitude"),
        title=paste0(granularity_display_name, " Objective Granularity Scaling: Online Performance"),subtitle=paste0("Mean across all 5000 episodes over middle 90% of experiment repetitions"))
