...
included_granularity_levels <- (
  (activity_means$RewGranularity ==0) | (activity_means$PenGranularity ==0) | (activity_means$Agent==ctrl_agent)
)

activity_means$GranularityLevel <- factor(paste0(
...
for (granularity_dim in c("RewGranularity","PenGranularity")){
  relevant_activity <- activity_means %>% 
    dplyr::filter(#ObjectiveDimChange %in% c("",granularity_dim) & 
      (activity_means[,granularity_dim,with=FALSE]==0 | is.na(activity_means[,granularity_dim,with=FALSE])) &
      (Agent %in% agents_to_show) & included_granularity_levels)
  ...
  baseplot <- ggplot(relevant_activity,aes(x=GranularityLevel,y=`R^*`,fill=Agent,color=Agent,group=interaction(Agent,GranularityLevel)))+
    ...
    facet_wrap(~EnvironmentClass,scales = "free_y")+theme(axis.text.x = element_text(angle=30),legend.position = "bottom")+
      labs(
        ...
        title=paste0(granularity_dim, " Scaling: Average Online Performance"),subtitle=paste("Mean across all episodes over ",max(relevant_activity$RunId)," experiment repetitions"))
